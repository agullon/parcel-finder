...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await utils.monitor_user_actions(f'{update.effective_user.first_name} {update.effective_user.last_name} joined for the first time')
...
async def InlineKeyboardHandler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if option == KEYBOARD_OPTIONS['navigate']:
        await utils.monitor_user_actions(f'{update.effective_user.first_name} {update.effective_user.last_name} requested instructions to go to a parcel')
        ...
        await utils.monitor_user_actions(f'{update.effective_user.first_name} {update.effective_user.last_name} requested a photo')
        sigpac_info, sigpac_error = utils.get_catastro_sigpac(poligono, parcela)
        if sigpac_error:
            await update.callback_query.edit_message_text(text=f'Ha habido un error: {sigpac_error}')
            context.user_data['poligono'] = None
        ...
        await utils.monitor_user_actions(f'{update.effective_user.first_name} {update.effective_user.last_name} started a new search')
        context.user_data['poligono'] = None
        context.user_data['parcela'] = None
        await update.callback_query.message.reply_text(text='Escribe el número de la parcela o envía tu ubicación para saber en qué parcela estás', reply_markup=search_keyboard)
...
async def text_input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if par_empty:
        await utils.monitor_user_actions(f'{update.effective_user.first_name} {update.effective_user.last_name} introduced parcela number')
        context.user_data['parcela'] = input
        await update.message.reply_text('Escribe el número del polígono')
        return
    elif pol_empty:
        await utils.monitor_user_actions(f'{update.effective_user.first_name} {update.effective_user.last_name} introduced polígono number')
        ...
        if error:
            ...
            await utils.monitor_user_actions(f'{update.effective_user.first_name} {update.effective_user.last_name} received parcela info')
...
async def location_input_handler(update, context):
    await utils.monitor_user_actions(f'{update.effective_user.first_name} {update.effective_user.last_name} sent location')
    pol_empty = True if not context.user_data.get('poligono') else False
    par_empty = True if not context.user_data.get('parcela') else False
